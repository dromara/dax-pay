...
/// # 分账接收方扫码授权（商户移动端）
///
/// 对照商户 Web 版 [MchAllocReceiverScanAuthController]，路径前缀 `/app-mch/channel/merchant/alloc-scan-auth`。
/// 商户号一律取自 [PaymentContext]，忽略请求中的 mchNo，防越权。
/// 移动端无渲染二维码场景, 生成授权链接后复制/转发给接收方, queryCode 轮询取结果。
@PermCode(menuCode = PermCodes.Channel.Merchant.MENU)
@Validated
@Tag(name = "分账接收方扫码授权服务(商户移动端)")
@RestController
@RequestMapping("/app-mch/channel/merchant/alloc-scan-auth")
@RequiredArgsConstructor
public class AppMerchantAllocReceiverScanAuthController {
