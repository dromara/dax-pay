...
/// appScope(档位) + appRefId(主键)调 [DouyinAppFacade#getById] 查得。
...
public class DouyinAuthStrategy implements ChannelAuthStrategy {
    ...
    /// 同时把 appScope/appRefId 写入 session, 供回调 doAuth 恢复凭证。
    @Override
    public AuthUrlResult generateAuthUrl(GenerateAuthUrlParam param, String authToken, AuthSession session) {
        DyAppView app = loadApp(param.getAppScope(), param.getAppRefId());
    ...
    public AuthResult doAuth(AuthCodeParam param, AuthSession session) {
        DyAppView app = loadApp(session.getAppScope(), session.getAppRefId());
    ...
    private DyAppView loadApp(String appScope, Long appRefId) {
        ...
        DyAppView app = douyinAppFacade.getById(scope, appRefId);
