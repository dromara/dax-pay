...
/// 供 [UnifiedAuthService] 按会话来源 O(1) 查找。
///
/// ## 注册机制
/// 每个实现以 `@Component` 注册, 通过 [#sourceCode] 声明对应的 [AuthSession] source 常量。
/// 消费方([UnifiedAuthService])注入 `List<PlatformAuthProvider>` 后按 sourceCode 建 Map 查找。
public interface PlatformAuthProvider {

    /// 认证来源标识
    ///
    /// 返回 [AuthSourceEnum] 枚举项, 由 [UnifiedAuthService] 按 [AuthSourceEnum#getCode] 索引 Provider。
