package cn.daxpay.open.payment.strategy.refund;
...
/// # 退款抽象策略
///
/// 策略为单例无状态, 运行时数据通过 [RefundOrder] 显式传递。
...
public abstract class AbsRefundStrategy implements PaymentStrategy {
    ...
    public void doBeforeRefund(RefundOrder refundOrder) {
    }

    /// 退款操作
    /// @param refundOrder 退款订单
    /// @return 退款结果(含映射后的退款状态)
    public abstract RefundResultBo doRefund(RefundOrder refundOrder);
