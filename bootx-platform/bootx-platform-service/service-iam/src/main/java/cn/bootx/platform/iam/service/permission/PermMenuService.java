...
public class PermMenuService {
    ...
    public PermMenuResult add(PermMenuParam param) {
        // 判断是否是一级菜单，是的话清空父菜单
        if (PermissionEnum.MENU_TYPE_TOP.equals(param.getMenuType())) {
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public PermMenuResult update(PermMenuParam param) {
        ...
        if (PermissionEnum.MENU_TYPE_TOP.equals(permMenu.getMenuType())) {
     ...
     * 根据菜单id获取资源(权限码)列表
     */
    public List<PermMenuResult> findResourceByMenuId(Long menuId) {
        ...
        List<PermMenu> resources = permMenuManager.findAllByParentId(menuId)
            .stream()
            .filter(permMenu -> Objects.equals(permMenu.getMenuType(), PermissionEnum.MENU_TYPE_RESOURCE))
