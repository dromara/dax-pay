...
import org.dromara.daxpay.service.pay.dao.notice.notify.MerchantNotifyRecordManager;
...
public class MerchantNotifySendService {
    ...
    public void send(Long taskId){
        ...
        if (taskOpt.isPresent()){
            ...
            boolean subscribe = notifyConfigService.getSubscribeByAppIdAndType(reqInfo.getAppId(), task.getNotifyType());
            if (Objects.equals(reqInfo.getNotifyType(), MerchantNotifyTypeEnum.HTTP.getCode()) && subscribe){
                this.sendData(task, reqInfo.getNotifyUrl(), LocalDateTime.now(), false);
            } else {
