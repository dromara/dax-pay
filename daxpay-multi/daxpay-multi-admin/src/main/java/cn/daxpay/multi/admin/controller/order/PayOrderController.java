...
public class PayOrderController {
    ...
    @GetMapping("/page")
    public Result<PageResult<PayOrderResult>> page(PageParam pageParam, PayOrderQuery param){
    ...
    @GetMapping("/findById")
    public Result<PayOrderResult> findById(Long id){
        PayOrderResult order = queryService.findById(id)
    ...
    public Result<PayOrderResult> findByOrderNo(String orderNo){
        PayOrderResult order = queryService.findByOrderNo(orderNo)
                .map(PayOrder::toResult)
                .orElseThrow(() -> new DataNotExistException("支付订单不存在"));
        return Res.ok(order);
    }
