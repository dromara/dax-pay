...
import cn.daxpay.open.channel.lakala.service.payment.isv.LakalaIsvPayService;
...
/// 配置组装在 [doBeforePay] 阶段完成(委托 [LakalaIsvConfigAssembler]), 支付执行委托给 [LakalaIsvPayService]。
///
/// 拉卡拉为聚合服务商模式, 不区分直连/服务商, 仅有一个产品 LAKALA_PAY。
@Slf4j
@Service
@RequiredArgsConstructor
public class LakalaIsvPayStrategy extends AbsNormalPayStrategy {

    private final LakalaIsvPayService lakalaIsvPayService;
    ...
    public PayTradeResultBo doPay(PayStrategyContext context) {
        ...
        return lakalaIsvPayService.pay(context.getTrade(), context.getPayParam(), credential);
    }
}
