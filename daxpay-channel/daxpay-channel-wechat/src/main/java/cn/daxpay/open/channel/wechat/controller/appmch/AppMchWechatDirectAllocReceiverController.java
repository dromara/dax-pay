...
/// # 微信直连分账接收方管理（商户移动端）
///
/// 对照商户 Web 版 [MchWechatDirectAllocReceiverController]，路径前缀 `/app-mch/wechat/direct-alloc-receiver`。
/// 商户号一律取自 [PaymentContext]，忽略请求中的 mchNo，防越权。
@PermCode(menuCode = PermCodes.Channel.Merchant.MENU)
@Validated
@Tag(name = "微信直连分账接收方管理(商户移动端)")
@RestController
@RequestMapping("/app-mch/wechat/direct-alloc-receiver")
@RequiredArgsConstructor
public class AppMchWechatDirectAllocReceiverController {

    private final WechatDirectAllocReceiverService allocReceiverService;
    ...
    private WechatDirectAllocReceiver loadOwned(Long id) {
        WechatDirectAllocReceiver entity = allocReceiverManager.findById(id)
