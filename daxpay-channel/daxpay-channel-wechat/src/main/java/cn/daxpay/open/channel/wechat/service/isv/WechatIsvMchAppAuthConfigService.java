...
public class WechatIsvMchAppAuthConfigService {
    ...
    ///
    /// 与 [#findByWechatIsvMchAppId] 的区别: 走 NotTenant 查询路径, 供认证策略(网关端无登录态)调用;
    /// [#save] 等配置态仍调原方法(保留租户隔离)。
    @IgnoreTenant
    @Transactional(rollbackFor = Exception.class)
    public WechatIsvMchAppAuthConfig findByWechatIsvMchAppIdForAuth(Long wechatIsvMchAppId) {
        var existing = wechatIsvMchAppAuthConfigManager.findByWechatIsvMchAppIdNotTenant(wechatIsvMchAppId);
        if (existing.isPresent()) {
            return existing.get();
        }
        var app = wechatIsvMchAppManager.findByIdNotTenant(wechatIsvMchAppId)
                // 微信: 服务商通道商户应用不存在
                .orElseThrow(() -> new DataNotExistException("error.channel.wechat.mchAppNotFound"));
        var config = new WechatIsvMchAppAuthConfig()
                .setChannelMchNo(app.getChannelMchNo())
