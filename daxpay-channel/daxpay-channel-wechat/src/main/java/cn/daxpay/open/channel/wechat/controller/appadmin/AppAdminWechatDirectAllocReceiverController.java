...
/// # 微信直连分账接收方管理(小程序管理端镜像)
///
/// 对应 admin 版 [WechatDirectAllocReceiverController], 全端点镜像, 复用同一 Service 与权限码。
@PermCode(menuCode = PermCodes.Channel.Merchant.MENU)
@Validated
@Tag(name = "小程序管理端-微信直连分账接收方管理")
@RestController
@RequestMapping("/app-admin/wechat/direct-alloc-receiver")
@RequiredArgsConstructor
public class AppAdminWechatDirectAllocReceiverController {
    ...
    public Result<Void> updateAlias(@NotNull(message = "{validation.field.id.notNull}") Long id,
        ...
        wechatDirectAllocReceiverService.updateAlias(id, alias);
        return Res.ok();
    }
}
