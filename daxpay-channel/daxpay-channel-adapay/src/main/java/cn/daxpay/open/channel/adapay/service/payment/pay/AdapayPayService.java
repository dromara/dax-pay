...
/// # Adapay 支付执行业务服务
///
/// 通过 [AdapayChannelClient] 调用子应用 dax-pay-channel-two 完成Adapay 支付下单。
/// 请求构建、响应解析全部在本类中完成。
///
/// Adapay 为聚合通道: 一个接口承载微信/支付宝/银联三种底层渠道, 由 pay_channel 决定路由。
...
public class AdapayPayService {
    ...
    public PayTradeResultBo pay(PayTrade order, NormalPayParam payParam, AdapaySdkCredential credential) {
        ...
        // 平台支付方式 → Adapay 通道支付方式(枚举 name 对齐, 直接 valueOf)
    ...
    /// 生成Adapay 支付异步通知地址(Adapay→平台)
    ///
    /// 路径约定: `{backendBaseUrl}/unipay/callback/{mchNo}/{channelMchNo}/adapay/pay`
    /// Adapay 回调验签只需平台公钥(全局固定), 凭 order_no 反查 PayTrade。
    ...
    private PayTradeResultBo toPayResult(AdapayPayResp resp) {
        PayTradeResultBo bo = new PayTradeResultBo()
                // Adapay 支付对象 ID, 后续 sync/close/refund 的关键凭证
