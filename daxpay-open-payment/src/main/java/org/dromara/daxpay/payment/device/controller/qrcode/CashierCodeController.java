package org.dromara.daxpay.payment.device.controller.qrcode;
...
public class CashierCodeController {
    ...
    @ClientCode(DaxPayCode.Client.MERCHANT)
    @RequestPath("绑定空白码牌")
    @Operation(summary = "绑定空白码牌")
    @PostMapping("/bindBlank")
    public Result<Void> bindBlank(@RequestBody @Validated CashierCodeBindBlankParam param){
        cashierCodeService.bindBlankCode(param);
        return Res.ok();
    }
    ...
    public ResponseEntity<byte[]> downloadByServer(@RequestBody @NotEmpty(message = "导出文件不可为空文件不可为空") List<Long> ids) {
        var bytes = cashierCodeService.exportBatch(ids);
        // 设置header信息
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
        headers.setContentDispositionFormData("attachment", URLEncoder.encode("码牌批量导出.xlsx", StandardCharsets.UTF_8));
