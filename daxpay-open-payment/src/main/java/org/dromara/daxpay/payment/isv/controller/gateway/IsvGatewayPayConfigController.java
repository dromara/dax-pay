...
@Tag(name = "网关支付配置(服务商)")
@RestController
@RequestMapping("/isv/gateway/pay/config")
...
public class IsvGatewayPayConfigController {
    ...
    public Result<IsvGatewayPayConfigResult> getConfig(@NotBlank(message = "服务商号不可为空") String isvNo) {
        return Res.ok(gatewayPayConfigService.getConfig(isvNo));
    }
