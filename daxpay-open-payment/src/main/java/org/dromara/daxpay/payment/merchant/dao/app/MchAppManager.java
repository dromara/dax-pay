package org.dromara.daxpay.payment.merchant.dao.app;
...
public class MchAppManager extends BaseManager<MchAppMapper, MchApp> {
     ...
     * 查询默认应用
     */
    public Optional<MchApp> findDefaultByMchNo(String mchNo) {
        return lambdaQuery()
                .eq(MchApp::isDefaultApp, true)
     ...
     */
    @IgnoreTenant
    public Optional<MchApp> findDefaultByMchNoNotTenant(String mchNo) {
        return lambdaQuery()
                .eq(MchApp::isDefaultApp, true)
                .eq(MchApp::getMchNo, mchNo)
                .oneOpt();
