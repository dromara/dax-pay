...
import org.dromara.daxpay.payment.isv.dao.gateway.IsvGatewayPayConfigManager;
...
public class IsvGatewayPayConfigService {
     ...
     * 获取网关支付配置
     */
    public IsvGatewayPayConfigResult getConfig(String isvNo){
        var gatewayPayConfig = gatewayPayConfigManager.findByIsvNo(isvNo);
        if (gatewayPayConfig.isEmpty()){
            var payConfig = new IsvGatewayPayConfig();
            payConfig.setIsvNo(isvNo);
            payConfig.setAggregateShow(true)
                    .setH5AutoUpgrade(false);
