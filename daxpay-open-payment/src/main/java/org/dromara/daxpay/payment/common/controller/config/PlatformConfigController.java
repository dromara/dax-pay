package org.dromara.daxpay.payment.common.controller.config;
...
@RequestGroup(groupCode = "PlatformConfig", groupName = "支付平台配置", moduleCode = "DaxPayPlatform", moduleName = "(DaxPay)支付平台")
...
public class PlatformConfigController {
    ...
    @IgnoreAuth
    @Operation(summary = "获取平台访问地址配置")
    @GetMapping("/url/getOpen")
    public Result<PlatformUrlConfigResult> getOpenUrl() {
        var urlConfig = platformConfigService.findUrlConfig();
        urlConfig.setGatewayServiceUrl(null);
        return Res.ok(urlConfig);
    }
    ...
    public Result<PlatformIntegrationConfigResult> getIntegrationConfig() {
        return Res.ok(platformConfigService.findIntegrationConfig());
    }

    @RequestPath("更新集成配置")
    @Operation(summary = "更新集成配置")
    @PostMapping("/integration/update")
    public Result<Void> updateIntegration(@RequestBody @Validated PlatformIntegrationConfigParam param) {
        platformConfigService.updateIntegration(param);
        return Res.ok();
    }
