package org.dromara.daxpay.payment.pay.util;

import org.dromara.daxpay.payment.common.exception.UnsupportedAbilityException;
import org.dromara.daxpay.payment.pay.strategy.PaymentStrategy;
...
public class PaymentStrategyFactory {
    ...
    public <T extends PaymentStrategy> T create(String channel, Class<T> clazz) {
        ...
        return beansOfType.values().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new UnsupportedAbilityException("不支持的能力: "+channel));
    }
    ...
    public <T extends PaymentStrategy> boolean existsByChannel(String channel, Class<T> clazz) {
        var beansOfType = SpringUtil.getBeansOfType(clazz);
        return beansOfType.values().stream().anyMatch(strategy -> strategy.getChannel().equals(channel));
    }
